...
function my_init_styles() {
	...
	if( $udesign_options['google_web_fonts_assoc'] ) {
	    ...
	    if( $google_fonts_string )
		    printf("<link rel='stylesheet' href='https://fonts.googleapis.com/css?family=%s&subset=latin,cyrillic' type='text/css' />\r\n", str_replace(' ', '+', $google_fonts_string));
	}
...
function customized_featured_image( $post_id, $img_src, $width, $height, $image_alignment = 'alignleft', $linked = true ) {
    $the_image_html = '<img src="'.get_bloginfo("template_directory").'/scripts/timthumb.php?src='.$img_src.'&amp;w='.$width.'&amp;h='.$height.'&amp;zc=1&amp;q=90" alt="' . esc_attr( get_post_field( 'post_title', $post_id ) ) . '" />';
...
<?php               else : ?>
                        <img class="hover-opacity" src="<?php echo get_bloginfo("template_directory"); ?>/scripts/timthumb.php?src=<?php echo $post_image_url; ?>&amp;w=570&amp;h=172&amp;zc=1&amp;q=90" alt="<?php the_title_attribute(); ?>" />
<?php               endif; ?>
                </div>
            </div>
<?php   endif;
...
require_once( TEMPLATEPATH . '/scripts/notifier/default-notifier.php' );
...
function udesign_more_link( $more_link, $more_link_text ) {
        $html .= ' <a title="'.$more_link_text.'" href="'.get_permalink().'" class="read-more-align-left"><span>'.$more_link_text.'</span> &rarr;</a>';
        $html .= '<div class="clear"></div>';
        return $html;
}
